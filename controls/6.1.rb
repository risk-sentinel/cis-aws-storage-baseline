...
control 'C-6.1' do
  ...
  # override else attestation_uri(:boundary, 'C-6.1'); empty -> Skip (stays
  ...
  if uri.to_s.empty?
    describe 'Ensure Elastic Disaster Recovery is Configured (attestation-required)' do
      skip "attestation-required: 'Ensure Elastic Disaster Recovery is Configured' is a setup/operational procedure not assertable via the AWS API. Set boundary_docs_base / c_6_1_attestation_uri to the configuration/operational evidence record, or supply a CMS-pattern attestation via `saf attest apply`."
    end
  else
    doc = document_attestation(uri, max_age_days: max_age_days)
    describe "C-6.1 evidence (#{uri})" do
