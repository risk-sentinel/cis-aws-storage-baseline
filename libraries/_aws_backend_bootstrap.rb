...
# and `Dir.pwd` is whatever cinc-auditor's WORKDIR is (`/work` in CI),
# not the profile directory — so neither `__dir__`-relative expansion nor
# `require_relative` can locate the vendor tree reliably.
#
# Instead: glob Dir.pwd for any vendored libraries directory, unshift onto
# $LOAD_PATH, and `require "aws_backend"` once. Ruby's require-once
# semantics means no duplicate load even if this file is loaded twice.
#
# The leading underscore in the filename sorts us first in InSpec's
# alphabetical library-load order (inspec-core profile_context.rb
# load_libraries — `libs.sort_by! { |l| l[1] }`), so sibling files
